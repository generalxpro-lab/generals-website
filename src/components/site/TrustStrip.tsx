...
const items = [
  { icon: ShieldCheck, title: "Secure Shopping", text: "Protected browsing and checkout" },
  { icon: Lock, title: "SSL Protected", text: "Encrypted connections" },
  { icon: RotateCcw, title: "Easy Returns", text: "Review our return policy" },
  ...
  { icon: BadgeCheck, title: "Quality Service", text: "Support when you need it" },
];
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-2 gap-x-4 gap-y-6 px-4 py-10 sm:grid-cols-3 lg:grid-cols-6 lg:px-8">
        {items.map((it, i) => (
          <Reveal key={it.title} delay={i * 60} className="flex min-w-0 items-start gap-3">
            <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-accent/10 text-accent"><it.icon size={19} /></span>
