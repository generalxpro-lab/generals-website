# Shanzen Enterprises Showcase

Build a premium, modern eCommerce demo website for Shanzen Enterprises using https://duranishop.com/ only as a visual inspiration for layout, spacing, animations, and overall shopping experience. Do not copy copyrighted images, logos, or text directly. Create an original website with a very similar premium feel.

Company Information

 Business Name: Shanzen Enterprises

 Address: 30 N Gould St Ste R, Sheridan, WY 82801, United States

 Display the address in the footer and contact page.

 Add a prominent Call Now button in the header and as a floating mobile button.

 This is a demo online store only. No real checkout or payment processing. Buttons may navigate between pages or display placeholder messages.

Design

 Luxury modern eCommerce appearance inspired by the reference.

 Glassmorphism effects.

 Smooth scrolling.

 Premium hover animations.

 Large hero banner.

 Sticky navigation.

 Rounded product cards.

 Modern typography.

 High-quality royalty-free lifestyle/product images similar in style to the reference.

 Fully responsive for desktop, tablet, and mobile.

Pages

 Home

 Shop

 Product Details

 Categories

 Deals

 Reviews

 About

 Contact

 FAQ

 Privacy Policy

 Terms & Conditions

Homepage

Include:

 Hero slider

 Featured products

 Best sellers

 Trending products

 Flash sale

 Limited-time offer banner

 New arrivals

 Category grid

 Newsletter signup

 Customer reviews

 Trust badges

 Footer

Floating Promotional Elements

Create floating animated promotional cards such as:

 20% OFF

 Free Shipping

 Secure Checkout

 Limited Offer

 New Arrival

 Best Seller

Payment Icons

Display animated payment badges:

 Visa

 Mastercard

 American Express

 Discover

 PayPal

 Apple Pay

 Google Pay

Product Cards

Each product should include:

 Large product image

 Product name

 Price

 Discount percentage

 Old price

 Star rating

 Review count

 Stock indicator

 View Details button

Product Details Page

When clicking a product:

 Large gallery

 Zoom effect

 Multiple thumbnails

 Description

 Features

 Specifications

 Shipping information

 Return policy

 Related products

 Customers also bought

 Similar items

Reviews

Display a realistic mix of customer feedback instead of only perfect ratings.
Example distribution:

 5★ = 70%

 4★ = 18%

 3★ = 7%

 2★ = 3%

 1★ = 2%

Each review should have a professional business response underneath.

Trust Elements

Include:

 Secure Shopping badge

 SSL Protected

 Money Back Guarantee

 Fast Shipping

 Customer Support

 Satisfaction Guarantee

Animations

 Fade-in sections

 Hover lift

 Card scaling

 Image zoom

 Counter animations

 Smooth page transitions

 Floating offer badges

 Animated CTA buttons

Footer

Include:

 Company information

 Address

 Contact section

 Social icons

 Newsletter

 Quick links

 Copyright

 Payment icons

Technical

 SEO-friendly HTML

 Lazy-loaded images

 Optimized performance

 Accessibility support

 Responsive design

Important: Use the reference website only as design inspiration. Create an original implementation with different colors, spacing, components, icons, typography, and royalty-free images.

One more thing: if this site is intended to help build credibility for Amazon wholesale or distributors, an online retail store may actually work against that goal. Many brands and distributors prefer to see a professional company website with an "About Us," business information, wholesale capabilities, and contact details—not a fake consumer storefront. If the objective is getting approved by brands, a business-focused website is usually the stronger choice.

This project was built with [Lovable](https://lovable.dev).

**Live app**: https://shanzen-luxe-showcase.lovable.app

## Build with Lovable

Continue developing this project in the [Lovable editor](https://lovable.dev/projects/65a43671-33d6-4e35-b1b5-bc9c5a046264).

- **Ship faster**: describe what you want to build and Lovable handles the code.
- **Stay in sync**: every change made in Lovable is committed straight to this repository.
- **Full ownership**: this code is yours. Push to `main` on GitHub and your changes sync back into Lovable, ready for your next prompt.

## Development

Prefer working locally? You need Node.js and npm — [install with nvm](https://github.com/nvm-sh/nvm#installing-and-updating).

```sh
git clone <this-repository-url>
cd <repository-name>
npm i
npm run dev
```
